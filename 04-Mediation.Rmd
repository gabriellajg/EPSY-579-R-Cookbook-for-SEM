...
```{r, eval = T}
labData <- read.table(file = "eatingDisorderSimData.csv", header = T, sep = ",")
```

Save the sample covariance matrix: 
...
which returns a 95% BCa confidence interval. 
...
ex4MediationSyntax <- "
	#Regressions
	DietSE ~ a*BMI                  	#Label the a coefficient in the M regression.
	Risk ~ cPrime*BMI + b*DietSE   #Label the direct effect (cPrime) of X and direct effect of M (b) in the Y regression.
